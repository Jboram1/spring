  ...
  <script>
  	...
  	$(function(){
  		$('#summernote').summernote({
	  	 ...
	  	 callbacks: { //여기 부분이 이미지를 첨부하는 부분 //썸머노트에서 제공하는 부분. 그냥 넣으면 됨. 하담함수호출만 수정
  	...
  	function uploadImage(file,this_location){ //매개변수 - file:이미지파일, 현재위치값
  		var fdata = new FormData(); //ajax에서 전송할때 form형태로 전송
  			fdata.append("file",file); //file이미지를 첨부 //ajax 안에 넣든 밖에 넣든 상관없음
  		$.ajax({
  			url:"/uploadImage",
  			type:"post",
  			dataType:"text", //파일을 받는형태 - text,json,xml...
  			data:fdata,
  			cache:false,		//남아 있는 데이터 정보 새롭게 정리. 캐시에 남아 있는 데이터 정보 다 정리하고 보내줘, 받아줘.
  			contentType:false, //파일을 보내는 형태. "json"으로도 보낼 수 있음. default값 : application/x-www-form-urlencoded //false로쓰지않으면 이미지파일이 안보내짐
  			processData:false, //query String 형태로 보내지 않음, ex)http://localhost:8000/bno=1&
  			enctype:"multipart/form-data",
  			success:function(data){ //서버에 가서 이미지를 저장후 파일이름과 저장위치를 리턴
  				//data : /upload/4681321_w1.jpg  //리턴받으면 파일명이 이렇게 바뀜
  				$(this_location).summernote('editor.insertImage',data); //js에서 함수호출해서 위치값에 데이터를 넣어줌.  //'editor.insertImage'썸머노트에서 지정함. 고정
