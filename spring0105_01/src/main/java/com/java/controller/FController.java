...
public class FController {
	...
	public String screenInfo(String movie) throws Exception {
		...
		//영화정보 api 가져오기
		String key = "fc0ad802c071157d5101a3c797d06b14";
		//오늘날짜
	...
	public String uploadImage(@RequestPart MultipartFile file) throws Exception {
		...
		if(!file.isEmpty()) {
			...
			file.transferTo(f); //파일서버로 전송 //add throws Exception 해줌
			urlName = "/upload/"+uploadFileName;
			System.out.println("FC ajax전송 링크주소 : "+urlName);
		}
		return urlName;
	}
	...
	public String bwrite(BoardDto bdto, @RequestPart MultipartFile file, Model model) throws Exception {
		System.out.println("FC bwrite btitle : "+bdto.getBtitle());
		//파일서버로 전송하는 부분
		if(!file.isEmpty()) {
			String oriFileName = file.getOriginalFilename();
			long time = System.currentTimeMillis();
		...
		System.out.println("FC bwrite bfile : "+bdto.getBfile());
